...
local function set_transfer_client(transfer)
	log.info({ "set_transfer_client: got transfer: ", transfer })
	local router = cartridge.service_get("vshard-router").get()
	transfer.bucket_id = router:bucket_id_mpcrc32(transfer.transfer_id)
	...
	if resp ~= nil and resp.error then
		log.error({ "set_transfer_client: storage error:", resp.error })
		return resp, nil
	end
...
local function set_transfer_state(transfer)
	log.info({ "set_transfer_state: got transfer: ", transfer })
	local router = cartridge.service_get("vshard-router").get()
	transfer.bucket_id = router:bucket_id_mpcrc32(transfer.transfer_id)
	...
	if error then
		log.error({ "set_transfer_state: request execution error:", error })
		return nil, error
	end

	if resp ~= nil and resp.error then
		log.error({ "set_transfer_state: storage error:", resp.error })
		return resp, nil
	end
		...
		if error or (resp ~= nil and resp.error) then
            ...
            if resp ~= nil and resp.error then
				...
				if resp.error.err == custom_errors.storageNotFoundErrors.AccNotFound.err then
					...
					local resp_state, error = set_transfer_state(transfer)
                    if error then
                        log.error({ "http_make_custom_transfer: request execution error:", error })
                        return internal_error_response(req, error)
                    end
                    if resp_state ~= nil and resp_state.error then
                        log.error({ "http_make_custom_transfer: storage error:", resp_state.error })
            ...
            elseif  isTransientError(error) then
                log.trace("transfer_id %s Retrying after error: %v", transfer.transfer_id, error)
			else
			...
			--если все ок
			account_array[i]["balance"] = resp[3]
			account_array[i]["pending_transfer"] = resp[4]
			account_array[i]["pending_amount"] = resp[5]
			account_array[i]["Found"] = true
...
local function init(opts)
	...
	httpd:route({ path = "/transfer/custom/create", method = "POST", public = true }, http_make_custom_transfer)
