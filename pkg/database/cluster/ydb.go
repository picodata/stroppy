...
func NewYdbCluster(
	...
	if envConfigured() {
		...
		options = append(options, environ.WithEnvironCredentials(ydbContext))
	}
	if tlsCertFile := envTlsCertificateFile(); len(tlsCertFile) > 0 {
		llog.Infoln("YDB custom TLS certificate file: ", tlsCertFile)
		options = append(options, ydb.WithCertificatesFromFile(tlsCertFile))
	}

	database, err = ydb.Open(ydbContext, dbURL, options...)
	if err != nil {
...
func (ydbCluster *YdbCluster) MakeAtomicTransfer(
	...
	return ydbCluster.ydbConnection.Table().Do(ctx,
...
func (ydbCluster *YdbCluster) FetchBalance(
	bic string,
	ban string,
