...
func (op *Provider) setVariableBlock(instanceFileBody *hcl2.Body, cpu int,
	...
	flexInstanceImageOCIDBlock := instanceFileBody.AppendNewBlock("variable", []string{"flex_instance_image_ocid"})
	flexInstanceImageOCIDBody := flexInstanceImageOCIDBlock.Body()
	flexInstanceImageOCIDBody.SetAttributeTraversal("type", hcl.Traversal{
		hcl.TraverseRoot{
			Name: "map(string)",
		},
	})
...
eu-frankfurt-1  = "ocid1.image.oc1.eu-frankfurt-1.aaaaaaaaw4ap4pklk3lo5pls5rppt2vfhjvuukpi2fltc74ycmaz3w7bz2aq" 
	...
	flexInstanceImageOCIDBody.SetAttributeTraversal("default", euFrankfurt1)
...
func (op *Provider) prepare(template *provider.ClusterParameters, nodes int, wd string) (err error) {
	instanceFile := hcl2.NewEmptyFile()
	instanceFileBody := instanceFile.Body()
	instanceFileBody.AppendNewline()

	nodesString := fmt.Sprintf("%d", nodes)
	diskString := fmt.Sprintf("%d", template.Disk)
	op.setVariableBlock(instanceFileBody, template.CPU, template.RAM, diskString, nodesString)
