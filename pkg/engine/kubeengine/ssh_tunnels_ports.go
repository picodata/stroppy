...
func (e *Engine) OpenPortForward(caller string, ports []string, reqURL *url.URL, stopPortForward chan struct{}) (err error) {
	...
	dialer := spdy.NewDialer(updater, &http.Client{Transport: httpTransaction}, http.MethodPost, reqURL)

	readyPortForward := make(chan struct{})

	var portForward *portforward.PortForwarder

	portForward, err = portforward.New(dialer, ports, stopPortForward, readyPortForward, portForwardLog, portForwardLog)
	if err != nil {
...
func (e *Engine) OpenSecureShellTunnel(caller string, mainPort int) (result *ssh.Result) {
	...
	for !engine.IsLocalPortOpen(tunnelPort) { // проверяем резервный порт в случае недоступности основного.
		tunnelPort++
	...
	tunnel, err = sshtunnel.NewSSHTunnel(mastersConnectionString, fmt.Sprintf("localhost:%v", mainPort), tunnelPort, authMethod)
	if err != nil {
		result = &ssh.Result{
			Port:   0,
			Tunnel: nil,
