...
func (pod *Pod) DeployNamespace() error {
	...
	if err = pod.e.FileToEngineObject(
		StroppyClientNSName,
		path.Join(
	...
	if err = pod.e.DeployObject(deployContext, func(clientSet *kubernetes.Clientset) error {
	...
	if err = pod.e.DeployAndWaitObject(
		...
		func(clientSet *kubernetes.Clientset) error {
			if err = clientSet.CoreV1().Pods(StroppyClientNSName).Delete(
				deployContext, StroppyClientPodName, pod.e.GenerateDefaultDeleteOtions(),
			); err != nil {
