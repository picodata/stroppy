...
run "Added user in docker group" "newgrp docker"
run "restarting docker service" "sudo service docker restart"
run "Restarted docker service" "docker login -u gitlab+deploy-token-489111 -p bzbGz3jwf1JsTrxvzN7x registry.gitlab.com"
run "Logged in repository" "sudo chown ubuntu:docker /var/run/docker.sock"
