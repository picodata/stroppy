...
// ClearTransferClient
// In case we failed for whatever reason try to clean up the transfer client, to allow speedy recovery.
...
func payWorkerCustomTx(
	...
	nTransfers int, zipfian bool, dbCluster CustomTxTransfer,
...
func (c *ClientCustomTx) RecoverTransfer(transferID model.TransferID) {
	...
	if err != nil {
		if err == cluster.ErrNoRows {
			llog.Errorf("[%v] [%v] Transfer not found when fetching for recovery", c.shortID, transferID)
		} else {
			llog.Errorf("[%v] [%v] Failed to fetch transfer: %v", c.shortID, transferID, err)
		}
