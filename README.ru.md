...
Как именно это позволяет проверить надёжность? Дело в том, что для проверки 
...
ведь в реальном мире отказы случаются гораздо чаще, чем нам хочется. Для 
...
Чтобы протестировать выбранную конфигурацию с помощью Stroppy мы можем 
...
запуска параметры через соответствующие флаги командной строки. Также нужно
подготовить указанные ниже файлы конфигурации.  
...
Например, такая конфигурация приведет к созданию кластера с тремя kubelet-нодами,
 у каждой из которых будет по 2 CPU и 8 GB ОЗУ.
```terraform
...
Также, необязательно задавать параметры именно в `vars.tf` — можно 
...
Но, если в процессе деплоя Stroppy обнаружит скрипт, то он применит именно
...
> (тредов), а не физических. Поэтому, указав cpu:2, мы фактически получили 4
...
Вне зависимости от того, запускаем ли мы Stroppy в Docker или локально, собрав из
исходников, аргументы передаваемые при запуске аргументы одни и те же. 
Например:
...
Для того чтобы задеплоить кластер кубернетеса в облачной
...
После вывода в консоль некоторой отладочной информации примерно через 
10-20 минут появится результат в виде сообщения такого вида:
```sh
...
дождаться ее выполнения. Результатом успешного выполнения теста будет примерно 
следующий вывод:
...
---
> **Note:** Указанные порты в интерактивном режиме (режим Stroppy, в котором
> он ожидает ввод следующей команды) — это порты по умолчанию для доступа к
> мониторингу (порт 3000) и доступа к API кластера k8s (6443). Так как Stroppy
...
вввести команду `pay` для старта теста переводов. Тест `Pay` будет запущен с 
...
---
> **Note:** Для варианта с тестированием FoundationDB, который мы
> запланировали после успешного деплоя, необходимо выполнить немного
> ручных манипуляций из пункта 2 раздела "Особенности использования".
---
...
<https://github.com/picodata/stroppy/issues>. Повторить тест можно еще одним 
запуском сначала `pop`, затем `pay`.
...
Для локального тестирования каких-либо новых фич (или просто для того чтобы 
...
1. Клонируем репозиторй `Stroppy` и подготавливаем разворачивание.
```shell
git clone https://github.com/picodata/stroppy.git && cd stroppy
make all
```

2. Запускаем кластер, в данном случае мы используем `Postgres`.
```shell
...
```
Если с кластером всё хорошо и он работает, мы должны увидеть нечто подобное:
```
minikube
type: Control Plane
host: Running
...
Чтобы эффективно использовать Stroppy, необходимо изучить набор доступных
команд и опций.
...
клиента нужно указать `client`. Для запуска интеграционных тестов — указать `local`.
...
(PostgreSQL), fdb (FoundationDB), mongodb (MongoDB), cocroach
(CockroachDB);  
...
`oracle`.  
`dir` — директория с файлами конфигурации. По умолчанию это текущая директория.
...
`count, n` — количество загружаемых счетов, по умолчанию 100000;  
`workers, w` — количество воркеров нагрузки (потоков-горутин), по умолчанию 
`4 * runtime.NumCPU()`;  
...
CocroachDB. Если ключ не задан, то размер пула равен количеству воркеров.
...
количества счетов (параметр `count`).  
Количество BAN определяется по следующей формуле:  
`Nban = (Nbic *brm)/square(count)`.  
Если Nban* Nbic > count, мы генерируем больше комбинаций (BIC, BAN), чем
мы сохранили во время процесса заполнения БД (это достигается если brm >
1). Рекомендуемый диапазон brm составляет от 1,01 до 1,1. Увеличение снижает
количество not found на тесте переводов, но увеличивает количество
dublicates на этапе загрузки счетов.  
...
и количество успешно загруженных записей совпадало с заданным.
...
В процессе выполнения тестов воркеры Stroppy могут получать различные
ошибки из-за проблем инфраструктуры или состояния СУБД. Для обеспечения
устойчивости теста, воркер, получивший на этапе тестов/отладки ошибку,
на некоторое время останавливается (до 10 миллисекунд), затем
увеличивает счетчик ```Retries``` — количество повторов, и выполняет
операцию с новыми сгенерированным счетом. Это происходит только в том
случае, если ошибка встречается в специальном пуле известных ошибок.
Таклй пул ошибок состоит как из общих ошибок, так и специфических для
тестируемой СУБД. Для изучения списка рекомендуется обратиться в [пакет
payload](https://github.com/picodata/stroppy/tree/main/internal/payload).
Если воркер получает ошибку, которой нет в пуле, он останавливает свою
работу с выводом в лог фатальной ошибки и увеличением счетчика
```Errors```.

Также внутри Stroppy определено несколько счетчиков для "логических" ошибок, 
что является штатным поведением в общем смысле, но фиксируются отдельно 
от общего количества операций:

`dublicates` — количество операций, получивших ошибку дублирования данных. 
Актуально для теста загрузки счетов.  
`Not found` — количество операций, завершившихся с ошибкой по причине того, 
что запись с переданным счетом не найдена в БД. Актуально для теста переводов.  
`Overdraft` — количество операций, завершившихся с ошибкой по причине того, 
что баланс счета-источника недостаточен для перевода указанной суммы. 
...
transfers — ключ transfer_id, значение которого генерируется пакетом 
...
в количестве создаваемых нод, в случае с Yandex.Cloud — создается по умолчанию.

- в деплое Oracle.Cloud есть дополнительный шан: монтирование отдельных 
...
генерируется по маске status_json_время_старта_cбора_статистики.json. Сбор
...
директориями файлов конфигурации. Это позволит избежать наложений и гибко 
