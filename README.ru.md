...
```shell
docker run -it docker-public.binary.picodata.io/stroppy:latest
```
...
5) Запускаем сборку stroppy.
```shell
go build -o bin/stroppy ./cmd/stroppy
```

6) Запускаем `stroppy` с параметрами для тестируемой базы данных
```shell
stroppy deploy --cloud yandex --flavor small --nodes 3 --dbtype ydb --log-level trace
```

> **Note:** Для запуска деплоя кластера в облаке в корне директории с файлами 
> конфигурации (пункт 2 выше) должны быть:  
> **Yandex.Cloud:**  
